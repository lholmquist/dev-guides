...
TODO: update paragraph to maybe make langchain.js sound better
...
END TODO: update paragraph for Node.js
...
Before you starting coding, you will need to install the applications dependencies and build the webui.  These include libraries like https://fastify.dev/[Fastify^] and https://js.langchain.com/docs/introduction/[Langchain.js^].

Run the following command in the VS Code terminal to change into the application directory, install applications dependencies and build the webui.
...
To add the new feature, you will either create or edit the following files.  You will also add a new dependency to the package.json. Use this list as a checklist to ensure you've made all the changes. If you get errors when you try to run the app, be sure each file was changed as described in the instructions below!

* `app/ai/email.mjs` - interface for interacting with the underlying LLM
* `app/routes/email-route.mjs` - A REST endpoint to interface with the email generator
* `app/server.mjs` - Main entry point for our Node.js application
* `app/webui/src/app/components/EmailGenerate/EmailGenerate.tsx` - A React web component providing a simple interface for providing a customer email
* `app/webui/src/app/routes.tsx` - A list of React routes to which you'll add the new Email generator component
* `app/package.json` - Important information about the application, like which dependencies to install

==== Create The Node.js AI Service

Create a new file, `email.mjs` in the `app/ai` folder to serve as the AI Service connection point between your application and the LLM.  Copy the following code and paste it into the `email.mjs` file.

To create this file, right-click on the `app/ai` folder in the file tree, then select "New File..." and enter the name of the new file `email.mjs`. The empty file will open.

image::devhub/new-file-nodejs.png[new-file, 500]
...
export async function createChainAndAskQuestion(model, userResponse) {
  ////////////////////////////////
  // CREATE CHAIN
  const prompt = ChatPromptTemplate.fromTemplate(`<1>
  ...
  const outputParser = StructuredOutputParser.fromZodSchema(<2>
    z.object({
  ...
  const chain = prompt.pipe(model).pipe(outputParser);<3>

  return await chain.invoke({<4>
    input: userResponse,
...
<1> Create a prompt to inform our LLM what do to.
<2> Define the schema for our Output Parser.
<3> Combine the prompt, model and outparser to create a runnable chain
<4> Run the chain we created to interact with the LLM
...
==== Install the Zod node module

In the above code, we created a schema to define what our output parser should look like.  It uses a popular schema validation module called https://zod.dev/[Zod^].  To use its capabilities, it needs to be installed and added to our package.json.

Run the following command in the VS Code terminal to install this new dependency and update the package.json
...
The update dependencies section of the app/package.json should look similar:
...
<1> - The newly added module

==== Create Fastify REST resource

Create a new Node.js file, `email-route.mjs` in the `app/routes` folder to serve a new REST endpoint. Copy the following code and paste it into the `email-route.mjs` file.
...
async function emailRoute(fastify, options) {
  fastify.post('/api/email', async (request, reply) => {<1>
    // AI Related Setup
     // AI Related Setup
    const model = getModel({ <2>
        modelName: 'parasol-instruct',
        baseURL: 'http://parasol-instruct-predictor.aiworkshop.svc.cluster.local:8080/v1'
    });
...
<2> pass the modelName and baseURL options to get a new model

==== Add the New REST resource to the application

Open the `server.mjs` file in the `app/` folder and add the following lines to register our newly created REST endpoint:

First we need to import the newly created file, which should be added around line 11 after the import of the claims route
...
Next, register the endpoint with the Fastify server by adding the following code around line 45, which is near the other endpoint registrations:
...
Create a new `EmailGenerate` folder in the `app/webui/src/app/components` folder. Also, create a new TypeScript XML file, `EmailGenerate.tsx` in the `app/webui/src/app/components/EmailGenerate` folder to render the output of the LLM in the email subject and body content. Copy the following code and paste it into the EmailGenerate.tsx file.
...
Open the `routes.tsx` file in the `app/webui/src/app` folder and `uncomment` the following code out in line *8* and *89 - 95* to show the new menu item.
...
TODO: update for Node.js local testing and instructions for building the Webui again.  maybe this is part of the nodemon run?

Before committing, do a quick functional test locally to ensure you got all the changes correct. We first need to rebuild the Web UI, so stop the nodemon process if it isn't already stopped.  Then run the following command to rebuild the UI:

[NOTE]
====
Press kbd:[Ctrl+c] in the terminal window to stop the running process.
====
...
Once the UI finishes rebuild, run the nodemon process again using this command:
...
* TODO: This whole next section needs the environment to test and figure out
...
TODO: image needs an update for Node.js.  needs to list the NOde.js files we added/updated instead of the java ones
...
TODO: needs an update for Node.js.  needs to show the Node.js files instead of the Java ones
...
TODO: needs an update for NOde.js.  currently shows a maven pipeline tasks that needs to be "npm install" or something similar
...
Reload the https://parasol-app-{user}-dev-parasol-app-{user}-dev.{openshift_cluster_ingress_domain}[Parasol web page^] in `production` to verify everything is working as expected. Ensure *not to reload the local page* when you were testing using Nodemon.
...
* END TODO: This whole next section needs the environment to test and figure out
