...
Run the following command in the VS Code terminal to install the applications dependencies and build the webui.
...
* `ai/email.mjs` - interface for interacting with the underlying LLM
* `routes/email-route.mjs` - A REST endpoint to interface with the email generator
* `webui/src/app/components/EmailGenerate/EmailGenerate.tsx` - A React web component providing a simple interface for providing a customer email
* `webui/src/app/routes.tsx` - A list of React routes to which you'll add the new Email generator component
* `package.json` - Important information about the application, like which dependencies to install

==== Create The Node.js AI Service

Create a new file, `email.mjs` in the `ai` folder to serve as the AI Service connection point between your application and the LLM.  Copy the following code and paste it into the `email.mjs` file.

To create this file, right-click on the `ai` folder in the file tree, then select "New File..." and enter the name of the new file `email.mjs`. The empty file will open.
...
The update dependencies section of the package.json should look similar:
...
Create a new Node.js file, `email-route.mjs` in the `routes` folder to serve a new REST endpoint. Copy the following code and paste it into the `email-route.mjs` file.
...
// Probably don't need any LLM model config updates, but not sure.  either way, it won't be this file
...
Create a new `EmailGenerate` folder in the `webui/src/app/components` folder. Also, create a new TypeScript XML file, `EmailGenerate.tsx` in the `webui/src/app/components/EmailGenerate` folder to render the output of the LLM in the email subject and body content. Copy the following code and paste it into the EmailGenerate.tsx file.
...
Open the `routes.tsx` file in the `webui/src/app` folder and `uncomment` the following code out in line *8* and *89 - 95* to show the new menu item.
...
Before committing, do a quick functional test locally to ensure you got all the changes correct. If you stopped the Nodemon process, start it again.
